...
class SideBar(QWidget):
    def __init__(self, parent):
        ...
        self.set_bg_color(QColor('lightgreen'))
        self.radios = self.get_radions()
        self.num_btn = QPushButton('Поменять количество точек', self)
        self.reset_btn = QPushButton('Стереть все', self)
    ...
    def render_buttons(self):
        ...
        layout.addWidget(QLabel('Цвет границы:', self))
        layout.addWidget(self.border_color_btn)
        layout.addWidget(QLabel('Цвет фона:', self))
        layout.addWidget(self.bg_color_btn)
        layout.addWidget(QLabel('Фигура:', self))
